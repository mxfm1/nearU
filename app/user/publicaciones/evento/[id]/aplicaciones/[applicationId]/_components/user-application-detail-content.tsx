...
export function UserApplicationDetailContent({
  eventId,
  applicationId,
}: UserApplicationDetailContentProps) {
  ...
  const statusConfig = application?.status
    ? getStatusConfig(application.status as ApplicationStatus)
    : null;
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 bg-card border-b shadow-sm">
        <nav className="flex justify-between items-center px-4 md:px-8 w-full h-16 max-w-6xl mx-auto">
          <Link href="/user/publicaciones" className="font-bold text-xl text-primary">
            NearU
          </Link>
          <Button variant="ghost" asChild>
            <Link href={`/user/publicaciones/evento/${eventId}/aplicaciones`}>
              <ArrowLeft className="w-4 h-4 mr-2" />
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 py-8 pb-32">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-grow space-y-6">
            ...
            <motion.div
              {...fadeInUp}
              className="bg-card rounded-xl p-6 shadow-sm flex flex-col md:flex-row items-center gap-6 border"
            >
              <div className="w-24 h-24 rounded-lg bg-muted overflow-hidden border shrink-0">
                {(application.applicantProfile as any)?.logoUrl ||
                (application.applicantProfile as any)?.imageUrl ? (
                  <img
                    src={
                      (application.applicantProfile as any)?.logoUrl ||
                      (application.applicantProfile as any)?.imageUrl
                    }
                    alt={application.applicantProfile?.name || ''}
              ...
              <div className="flex-grow text-center md:text-left">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-x-4 gap-y-1 text-muted-foreground">
                  <span className="flex items-center gap-1">
                    {(application.applicantProfile as any)?.categories?.[0] ||
                      (application.applicantProfile as any)?.industry ||
                      'Proveedor'}
                  </span>
                  <span className="flex items-center gap-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-1">
                  ...
                  <p className="text-base text-foreground">
                    {application.applicantProfile?.name || 'No disponible'}
                  </p>
                </div>
                {/* <div className="space-y-1">
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <p className="text-base text-muted-foreground leading-relaxed">
                    {application.coverLetter ||
                      'El proveedor no incluyó una carta de presentación.'}
                  </p>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              {...fadeInUp}
              className="bg-card rounded-xl p-6 shadow-sm border flex flex-col sm:flex-row items-center justify-between gap-4"
            >
              <div className="hidden md:flex flex-col">
                <p className="text-sm text-muted-foreground">Evaluando postulación de:</p>
                <p className="text-lg font-semibold leading-tight">
                  {application.applicantProfile?.name || 'Proveedor'}
                </p>
              </div>
            ...
            >
                  ...
                  application.score.breakdown.length > 0 ? (
                    application.score.breakdown.map((item: any, idx: number) => {
                      const itemPercentage =
                        item.pointsPossible > 0
                          ? (item.pointsEarned / item.pointsPossible) * 100
                          : 0;
                      return (
                        <div key={idx} className="space-y-2">
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-muted-foreground">
                              {item.ruleType || `Criterio ${idx + 1}`}
                            </span>
                    ...
                    <div className="text-center py-4">
                      <p className="text-sm text-muted-foreground">
                        No hay criterios de evaluación configurados para este evento.
                      </p>
                    </div>
                  )
                ) : (
              ...
              <div className="mt-6 p-4 bg-primary/5 rounded-lg border border-primary/10">
                ...
                <p className="text-xs text-muted-foreground italic">
                  La postulación ha sido evaluada automáticamente por el sistema de scoring de NearU
                  en base a los parámetros elegidos al crear el evento.
                </p>
              </div>
            </motion.div>
