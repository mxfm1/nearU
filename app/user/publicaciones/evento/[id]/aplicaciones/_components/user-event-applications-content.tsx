...
function getStatusBadge(status: ApplicationStatus) {
  switch (status) {
    case 'pending':
      return (
        <span className="px-4 py-1.5 rounded-full bg-muted text-zinc-600 font-bold text-sm border border-border">
          Pendiente
        </span>
      );
    case 'reviewing':
...
function ApplicationCard({ application }: { application: EventApplication }) {
  ...
  const profileName =
    appAny.applicantProfile?.name || appAny.profileName || appAny.name || 'Proveedor';
  const profileImage =
    appAny.applicantProfile?.logoUrl || appAny.profileImageUrl || appAny.imageUrl;
  ...
  return (
    <div
      onClick={() =>
        router.push(
          `/user/publicaciones/evento/${application.eventId}/aplicaciones/${application.id}`
        )
      }
        ...
        <div className="flex-grow">
          ...
          <div className="flex flex-wrap gap-2">
            {profileCategories.map((cat: string) => (
              <span
                key={cat}
                className="bg-muted px-3 py-0.5 rounded-full text-sm text-muted-foreground"
              >
                {cat}
              </span>
            ))}
          ...
          <div className="text-center">
            ...
            <div
              className={cn(
                ...
                totalScore >= 80 ? 'bg-amber-100 text-amber-700' : 'bg-muted text-muted-foreground'
              )}
            >
              <Star className={cn('w-4 h-4', totalScore >= 80 && 'fill-amber-500')} />
              {totalScore} pts
            </div>
          </div>
          <div className="text-center">
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4">
      ...
      <p className="text-muted-foreground text-center max-w-md mb-6">
        Este evento no ha recibido postulaciones de proveedores. Cuando los proveedores apliquen,
        podrás ver sus perfiles aquí.
      </p>
      <Button asChild>
...
export function UserEventApplicationsContent({
  eventId,
  statusFilter,
  currentPage,
}: UserEventApplicationsContentProps) {
  const router = useRouter();
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-40 h-32 md:h-auto rounded-l-xl overflow-hidden shrink-0 bg-muted">
                {event.thumbnailUrl ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-primary/10 min-h-[120px]">
        ...
        {totalPages > 1 && applications.length > 0 && (
          <div className="mt-8 flex flex-col md:flex-row items-center justify-between bg-card p-4 rounded-xl border border-border">
            <span className="text-sm text-muted-foreground mb-4 md:mb-0">
              ...
              <strong>{Math.min(currentPage * 10, total)}</strong> de <strong>{total}</strong>{' '}
              aplicaciones
            </span>
